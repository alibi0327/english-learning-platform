window.A1_DATA = {
  "lessons": [
    {
      "key": "a1-01",
      "title": "Greetings & Introductions",
      "ru": "Приветствия и знакомство",
      "theory": [
        "В английском языке приветствие зависит от ситуации. Hello — нейтральное приветствие, Hi — неформальное. Good morning используют утром, Good afternoon — днём, Good evening — вечером.",
        "Чтобы представиться, используйте My name is ... или I’m .... Чтобы спросить имя: What’s your name? Вежливый ответ после знакомства: Nice to meet you.",
        "How are you? означает «Как дела?». Простые ответы уровня A1: I’m fine, thanks. / I’m good, thank you. / I’m OK."
      ],
      "grammar": [
        [
          "Фраза",
          "Значение",
          "Пример"
        ],
        [
          "My name is ...",
          "Меня зовут ...",
          "My name is Anna."
        ],
        [
          "I’m ...",
          "Я ... / Меня зовут ...",
          "I’m Alex."
        ],
        [
          "What’s your name?",
          "Как тебя/вас зовут?",
          "What’s your name? — I’m Sam."
        ],
        [
          "Nice to meet you.",
          "Приятно познакомиться.",
          "Nice to meet you too."
        ]
      ],
      "examples": [
        [
          "Hello! My name is Anna.",
          "Привет! Меня зовут Анна."
        ],
        [
          "Hi, I’m Max. What’s your name?",
          "Привет, я Макс. Как тебя зовут?"
        ],
        [
          "Nice to meet you.",
          "Приятно познакомиться."
        ],
        [
          "How are you? — I’m fine, thanks.",
          "Как дела? — Хорошо, спасибо."
        ]
      ],
      "vocab": [
        [
          "hello",
          "/həˈləʊ/",
          "привет / здравствуйте",
          "Hello, Maria!"
        ],
        [
          "hi",
          "/haɪ/",
          "привет",
          "Hi, Ben!"
        ],
        [
          "name",
          "/neɪm/",
          "имя",
          "My name is Leo."
        ],
        [
          "meet",
          "/miːt/",
          "встречать; знакомиться",
          "Nice to meet you."
        ],
        [
          "fine",
          "/faɪn/",
          "хорошо",
          "I’m fine, thanks."
        ],
        [
          "thanks",
          "/θæŋks/",
          "спасибо",
          "Fine, thanks."
        ],
        [
          "morning",
          "/ˈmɔːnɪŋ/",
          "утро",
          "Good morning!"
        ],
        [
          "evening",
          "/ˈiːvnɪŋ/",
          "вечер",
          "Good evening!"
        ]
      ],
      "practice": [
        {
          "q": "Дополните: “___ name is Kate.”",
          "a": "My",
          "hint": "Фраза «Меня зовут...» начинается с My name is."
        },
        {
          "q": "Как спросить имя собеседника?",
          "a": "What’s your name?",
          "hint": "Используйте вопрос What’s ...?"
        },
        {
          "q": "Ответьте на “How are you?” одним простым предложением.",
          "a": "I’m fine, thanks.",
          "hint": "Возможны также I’m good / I’m OK."
        }
      ],
      "quiz": [
        {
          "q": "Как сказать «Приятно познакомиться»?",
          "options": [
            "Good evening",
            "Nice to meet you",
            "How old are you?",
            "See you morning"
          ],
          "answer": 1
        },
        {
          "q": "Выберите правильное: “___ name is Tom.”",
          "options": [
            "I",
            "Me",
            "My",
            "Mine"
          ],
          "answer": 2
        },
        {
          "q": "Что означает “What’s your name?”",
          "options": [
            "Сколько тебе лет?",
            "Откуда ты?",
            "Как тебя зовут?",
            "Как дела?"
          ],
          "answer": 2
        },
        {
          "q": "Какое приветствие неформальное?",
          "options": [
            "Hi",
            "Good afternoon",
            "Good evening",
            "Good morning"
          ],
          "answer": 0
        },
        {
          "q": "Подходящий ответ на “How are you?”",
          "options": [
            "My name is Ben.",
            "I’m fine, thanks.",
            "Good evening.",
            "Nice name."
          ],
          "answer": 1
        },
        {
          "q": "Как сказать «Добрый вечер»?",
          "options": [
            "Good morning",
            "Good afternoon",
            "Good evening",
            "Good night morning"
          ],
          "answer": 2
        },
        {
          "q": "“I’m Alex” можно использовать, чтобы представиться.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Как ответить на “Nice to meet you”?",
          "options": [
            "Nice to meet you too.",
            "My age is ten.",
            "Good morning night.",
            "What name?"
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-02",
      "title": "Alphabet & Spelling",
      "ru": "Алфавит и произношение по буквам",
      "theory": [
        "В английском алфавите 26 букв. Важно знать название каждой буквы, потому что по буквам часто диктуют имена, фамилии, адреса и email.",
        "Вопрос How do you spell it? означает «Как это пишется по буквам?». Ответ: A-L-E-X. Для имени можно спросить How do you spell your name?",
        "Обратите внимание на буквы, которые часто путают: E /iː/, I /aɪ/, G /dʒiː/, J /dʒeɪ/, R /ɑː/, Y /waɪ/."
      ],
      "grammar": [
        [
          "Группа",
          "Буквы / звучание",
          "Подсказка"
        ],
        [
          "/eɪ/",
          "A, H, J, K",
          "A = /eɪ/"
        ],
        [
          "/iː/",
          "B, C, D, E, G, P, T, V",
          "E = /iː/"
        ],
        [
          "/aɪ/",
          "I, Y",
          "I = /aɪ/, Y = /waɪ/"
        ],
        [
          "Вопрос",
          "How do you spell ...?",
          "Как ... пишется по буквам?"
        ]
      ],
      "examples": [
        [
          "How do you spell your name?",
          "Как пишется твоё имя по буквам?"
        ],
        [
          "A-L-I-B-I.",
          "A-L-I-B-I."
        ],
        [
          "My email is ...",
          "Мой email ..."
        ],
        [
          "Is that B or D?",
          "Это B или D?"
        ]
      ],
      "vocab": [
        [
          "alphabet",
          "/ˈælfəbet/",
          "алфавит",
          "The English alphabet has 26 letters."
        ],
        [
          "letter",
          "/ˈletə/",
          "буква",
          "A is a letter."
        ],
        [
          "spell",
          "/spel/",
          "произносить/писать по буквам",
          "Spell your name, please."
        ],
        [
          "word",
          "/wɜːd/",
          "слово",
          "This is a new word."
        ],
        [
          "email",
          "/ˈiːmeɪl/",
          "электронная почта",
          "What’s your email?"
        ],
        [
          "please",
          "/pliːz/",
          "пожалуйста",
          "Spell it, please."
        ]
      ],
      "practice": [
        {
          "q": "Продиктуйте по буквам имя “Anna”.",
          "a": "A-N-N-A",
          "hint": "Называйте каждую букву отдельно."
        },
        {
          "q": "Как спросить «Как это пишется по буквам?»",
          "a": "How do you spell it?",
          "hint": "Начните с How do you ...?"
        },
        {
          "q": "Сколько букв в английском алфавите?",
          "a": "26",
          "hint": "Twenty-six."
        }
      ],
      "quiz": [
        {
          "q": "Сколько букв в английском алфавите?",
          "options": [
            "24",
            "25",
            "26",
            "27"
          ],
          "answer": 2
        },
        {
          "q": "Что означает “spell”?",
          "options": [
            "считать",
            "произносить по буквам",
            "читать быстро",
            "переводить"
          ],
          "answer": 1
        },
        {
          "q": "Как спросить «Как пишется твоё имя?»",
          "options": [
            "What is your age?",
            "How do you spell your name?",
            "Where is your name?",
            "Who spell?"
          ],
          "answer": 1
        },
        {
          "q": "Как называется буква I?",
          "options": [
            "/iː/",
            "/aɪ/",
            "/eɪ/",
            "/waɪ/"
          ],
          "answer": 1
        },
        {
          "q": "Как называется буква Y?",
          "options": [
            "/waɪ/",
            "/aɪ/",
            "/wiː/",
            "/jeɪ/"
          ],
          "answer": 0
        },
        {
          "q": "Что означает “letter”?",
          "options": [
            "цифра",
            "слово",
            "буква",
            "адрес"
          ],
          "answer": 2
        },
        {
          "q": "A-L-E-X — это пример spelling.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Какое слово означает «пожалуйста»?",
          "options": [
            "please",
            "thanks",
            "spell",
            "word"
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-03",
      "title": "Numbers, Age & Phone Numbers",
      "ru": "Числа, возраст и номера телефонов",
      "theory": [
        "Сначала выучите 0–20: zero, one, two, three ... twenty. Затем десятки: thirty, forty, fifty, sixty, seventy, eighty, ninety, one hundred.",
        "Числа 21–99 между десятками образуются через дефис: twenty-one, thirty-five, ninety-nine.",
        "Возраст спрашивают How old are you? Ответ: I’m fifteen years old. В номерах телефонов цифры обычно читают по одной; 0 часто произносят zero или oh."
      ],
      "grammar": [
        [
          "Число",
          "English",
          "Число",
          "English"
        ],
        [
          "0",
          "zero",
          "10",
          "ten"
        ],
        [
          "12",
          "twelve",
          "20",
          "twenty"
        ],
        [
          "13",
          "thirteen",
          "30",
          "thirty"
        ],
        [
          "15",
          "fifteen",
          "50",
          "fifty"
        ],
        [
          "40",
          "forty",
          "100",
          "one hundred"
        ]
      ],
      "examples": [
        [
          "I’m sixteen years old.",
          "Мне шестнадцать лет."
        ],
        [
          "How old are you?",
          "Сколько тебе лет?"
        ],
        [
          "My phone number is 555-2018.",
          "Мой номер телефона 555-2018."
        ],
        [
          "There are thirty-two students.",
          "Там тридцать два ученика."
        ]
      ],
      "vocab": [
        [
          "number",
          "/ˈnʌmbə/",
          "число; номер",
          "What’s your phone number?"
        ],
        [
          "old",
          "/əʊld/",
          "лет; старый",
          "How old are you?"
        ],
        [
          "phone",
          "/fəʊn/",
          "телефон",
          "My phone is here."
        ],
        [
          "twenty",
          "/ˈtwenti/",
          "двадцать",
          "Twenty books."
        ],
        [
          "thirty",
          "/ˈθɜːti/",
          "тридцать",
          "Thirty days."
        ],
        [
          "forty",
          "/ˈfɔːti/",
          "сорок",
          "Forty people."
        ],
        [
          "hundred",
          "/ˈhʌndrəd/",
          "сто",
          "One hundred."
        ]
      ],
      "practice": [
        {
          "q": "Напишите по-английски 42.",
          "a": "forty-two",
          "hint": "40 = forty, 2 = two."
        },
        {
          "q": "Как спросить возраст?",
          "a": "How old are you?",
          "hint": "Используйте old."
        },
        {
          "q": "Напишите по-английски 13.",
          "a": "thirteen",
          "hint": "Не путайте с thirty."
        }
      ],
      "quiz": [
        {
          "q": "Как по-английски 40?",
          "options": [
            "fourty",
            "forty",
            "fourteen",
            "fourty-ten"
          ],
          "answer": 1
        },
        {
          "q": "Как сказать 21?",
          "options": [
            "twenty-one",
            "twelve-one",
            "two-one",
            "twenty-first"
          ],
          "answer": 0
        },
        {
          "q": "Что означает “How old are you?”",
          "options": [
            "Как тебя зовут?",
            "Сколько тебе лет?",
            "Какой твой номер?",
            "Где ты живёшь?"
          ],
          "answer": 1
        },
        {
          "q": "Как по-английски 13?",
          "options": [
            "thirty",
            "thirteen",
            "three-ten",
            "third"
          ],
          "answer": 1
        },
        {
          "q": "Как по-английски 50?",
          "options": [
            "fivety",
            "fifteen",
            "fifty",
            "five-zero"
          ],
          "answer": 2
        },
        {
          "q": "Как сказать «Мне 18 лет»?",
          "options": [
            "I have eighteen.",
            "I’m eighteen years old.",
            "My old is eighteen.",
            "I years eighteen."
          ],
          "answer": 1
        },
        {
          "q": "99 = ninety-nine.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Что означает “phone number”?",
          "options": [
            "номер телефона",
            "номер дома",
            "возраст",
            "имя"
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-04",
      "title": "Personal Pronouns & Verb To Be",
      "ru": "Личные местоимения и глагол to be",
      "theory": [
        "Личные местоимения: I — я, you — ты/вы, he — он, she — она, it — оно/это, we — мы, they — они.",
        "Глагол to be в настоящем времени имеет три формы: am, is, are. I am; he/she/it is; you/we/they are.",
        "Краткие формы очень распространены: I’m, you’re, he’s, she’s, it’s, we’re, they’re. Отрицание: I’m not, he isn’t, they aren’t."
      ],
      "grammar": [
        [
          "Местоимение",
          "Утверждение",
          "Отрицание"
        ],
        [
          "I",
          "I am / I’m",
          "I am not / I’m not"
        ],
        [
          "He / She / It",
          "is / ’s",
          "is not / isn’t"
        ],
        [
          "You / We / They",
          "are / ’re",
          "are not / aren’t"
        ]
      ],
      "examples": [
        [
          "I’m a student.",
          "Я ученик."
        ],
        [
          "She is my friend.",
          "Она моя подруга."
        ],
        [
          "They are at home.",
          "Они дома."
        ],
        [
          "He isn’t a teacher.",
          "Он не учитель."
        ]
      ],
      "vocab": [
        [
          "student",
          "/ˈstjuːdənt/",
          "ученик; студент",
          "I’m a student."
        ],
        [
          "teacher",
          "/ˈtiːtʃə/",
          "учитель",
          "She is a teacher."
        ],
        [
          "friend",
          "/frend/",
          "друг",
          "He is my friend."
        ],
        [
          "home",
          "/həʊm/",
          "дом; дома",
          "We are at home."
        ],
        [
          "happy",
          "/ˈhæpi/",
          "счастливый",
          "They are happy."
        ],
        [
          "tired",
          "/ˈtaɪəd/",
          "уставший",
          "I’m tired."
        ]
      ],
      "practice": [
        {
          "q": "Дополните: She ___ happy.",
          "a": "is",
          "hint": "С he/she/it используем is."
        },
        {
          "q": "Сделайте отрицание: They are at home.",
          "a": "They aren’t at home.",
          "hint": "are not = aren’t."
        },
        {
          "q": "Замените “Anna” местоимением.",
          "a": "she",
          "hint": "Для женщины/девушки — she."
        }
      ],
      "quiz": [
        {
          "q": "Выберите: I ___ a student.",
          "options": [
            "is",
            "are",
            "am",
            "be"
          ],
          "answer": 2
        },
        {
          "q": "Выберите: She ___ my friend.",
          "options": [
            "am",
            "is",
            "are",
            "be"
          ],
          "answer": 1
        },
        {
          "q": "Выберите: They ___ happy.",
          "options": [
            "is",
            "am",
            "are",
            "be"
          ],
          "answer": 2
        },
        {
          "q": "Какое местоимение означает «мы»?",
          "options": [
            "they",
            "we",
            "you",
            "it"
          ],
          "answer": 1
        },
        {
          "q": "Краткая форма “he is”",
          "options": [
            "he’re",
            "he’s",
            "he’m",
            "his"
          ],
          "answer": 1
        },
        {
          "q": "Отрицание “She is tired.”",
          "options": [
            "She not tired.",
            "She isn’t tired.",
            "She aren’t tired.",
            "She am not tired."
          ],
          "answer": 1
        },
        {
          "q": "С “you” используется are.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Какое местоимение подходит для “my parents”?",
          "options": [
            "he",
            "she",
            "it",
            "they"
          ],
          "answer": 3
        }
      ]
    },
    {
      "key": "a1-05",
      "title": "Questions with To Be",
      "ru": "Вопросы с глаголом to be",
      "theory": [
        "Чтобы сделать общий вопрос с to be, поставьте am/is/are перед подлежащим: You are ready. → Are you ready? She is a teacher. → Is she a teacher?",
        "Короткие ответы: Yes, I am. / No, I’m not. Yes, he is. / No, he isn’t. Yes, they are. / No, they aren’t.",
        "В специальных вопросах сначала идёт вопросительное слово: What is your name? Where are you from? How are you?"
      ],
      "grammar": [
        [
          "Тип",
          "Схема",
          "Пример"
        ],
        [
          "Общий вопрос",
          "Am/Is/Are + subject ...?",
          "Are you a student?"
        ],
        [
          "Короткий ответ",
          "+",
          "Yes, I am."
        ],
        [
          "Короткий ответ",
          "−",
          "No, I’m not."
        ],
        [
          "Специальный",
          "Question word + am/is/are ...?",
          "Where are they?"
        ]
      ],
      "examples": [
        [
          "Are you a student? — Yes, I am.",
          "Ты ученик? — Да."
        ],
        [
          "Is she from London? — No, she isn’t.",
          "Она из Лондона? — Нет."
        ],
        [
          "Where are you from?",
          "Откуда ты?"
        ],
        [
          "What is your name?",
          "Как тебя зовут?"
        ]
      ],
      "vocab": [
        [
          "where",
          "/weə/",
          "где; откуда",
          "Where are you from?"
        ],
        [
          "what",
          "/wɒt/",
          "что; какой",
          "What is your name?"
        ],
        [
          "who",
          "/huː/",
          "кто",
          "Who is he?"
        ],
        [
          "ready",
          "/ˈredi/",
          "готовый",
          "Are you ready?"
        ],
        [
          "from",
          "/frɒm/",
          "из; от",
          "I’m from Kazakhstan."
        ],
        [
          "country",
          "/ˈkʌntri/",
          "страна",
          "What country are you from?"
        ]
      ],
      "practice": [
        {
          "q": "Сделайте вопрос: You are ready.",
          "a": "Are you ready?",
          "hint": "Перенесите are в начало."
        },
        {
          "q": "Коротко ответьте «да» на “Is he a teacher?”",
          "a": "Yes, he is.",
          "hint": "Повторите he + is."
        },
        {
          "q": "Как спросить «Откуда ты?»",
          "a": "Where are you from?",
          "hint": "Where + are + you + from."
        }
      ],
      "quiz": [
        {
          "q": "Правильный вопрос к “You are happy.”",
          "options": [
            "You are happy?",
            "Are you happy?",
            "Is you happy?",
            "Do you are happy?"
          ],
          "answer": 1
        },
        {
          "q": "Ответ на “Are you a student?”",
          "options": [
            "Yes, I am.",
            "Yes, I is.",
            "Yes, you are.",
            "Yes, am."
          ],
          "answer": 0
        },
        {
          "q": "Как спросить «Кто он?»",
          "options": [
            "What he is?",
            "Who is he?",
            "Where he?",
            "Is who he?"
          ],
          "answer": 1
        },
        {
          "q": "Выберите: ___ she from Almaty?",
          "options": [
            "Am",
            "Are",
            "Is",
            "Be"
          ],
          "answer": 2
        },
        {
          "q": "Отрицательный короткий ответ для “Are they ready?”",
          "options": [
            "No, they isn’t.",
            "No, they not.",
            "No, they aren’t.",
            "No, aren’t."
          ],
          "answer": 2
        },
        {
          "q": "Что означает “Where are you from?”",
          "options": [
            "Где ты сейчас?",
            "Откуда ты?",
            "Кто ты?",
            "Сколько тебе лет?"
          ],
          "answer": 1
        },
        {
          "q": "В вопросе с to be форма am/is/are ставится перед подлежащим.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Как спросить «Как тебя зовут?»",
          "options": [
            "What is your name?",
            "Where your name?",
            "Who name are you?",
            "How old name?"
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-06",
      "title": "Articles A / An & Plurals",
      "ru": "Артикли a/an и множественное число",
      "theory": [
        "A и an употребляются перед одним исчисляемым предметом: a book, a teacher, an apple, an engineer. A ставим перед согласным звуком, an — перед гласным звуком.",
        "Обычно множественное число образуется добавлением -s: book → books. После -s, -sh, -ch, -x часто добавляется -es: bus → buses, box → boxes.",
        "Некоторые формы неправильные: man → men, woman → women, child → children, person → people."
      ],
      "grammar": [
        [
          "Правило",
          "Единственное",
          "Множественное"
        ],
        [
          "обычно +s",
          "a book",
          "books"
        ],
        [
          "-s/-sh/-ch/-x +es",
          "a box",
          "boxes"
        ],
        [
          "согласный + y → ies",
          "a city",
          "cities"
        ],
        [
          "неправильное",
          "a child",
          "children"
        ]
      ],
      "examples": [
        [
          "This is a book.",
          "Это книга."
        ],
        [
          "She is an engineer.",
          "Она инженер."
        ],
        [
          "I have two boxes.",
          "У меня две коробки."
        ],
        [
          "There are three children.",
          "Там трое детей."
        ]
      ],
      "vocab": [
        [
          "book",
          "/bʊk/",
          "книга",
          "It is a book."
        ],
        [
          "apple",
          "/ˈæpəl/",
          "яблоко",
          "This is an apple."
        ],
        [
          "engineer",
          "/ˌendʒɪˈnɪə/",
          "инженер",
          "He is an engineer."
        ],
        [
          "box",
          "/bɒks/",
          "коробка",
          "Two boxes."
        ],
        [
          "child",
          "/tʃaɪld/",
          "ребёнок",
          "Three children."
        ],
        [
          "person",
          "/ˈpɜːsən/",
          "человек",
          "Five people."
        ]
      ],
      "practice": [
        {
          "q": "Выберите артикль: ___ apple.",
          "a": "an",
          "hint": "Apple начинается с гласного звука."
        },
        {
          "q": "Множественное число “box”.",
          "a": "boxes",
          "hint": "После x добавляем -es."
        },
        {
          "q": "Множественное число “child”.",
          "a": "children",
          "hint": "Это неправильная форма."
        }
      ],
      "quiz": [
        {
          "q": "Выберите: ___ book",
          "options": [
            "a",
            "an",
            "two",
            "are"
          ],
          "answer": 0
        },
        {
          "q": "Выберите: ___ apple",
          "options": [
            "a",
            "an",
            "the two",
            "is"
          ],
          "answer": 1
        },
        {
          "q": "Множественное число “book”",
          "options": [
            "bookes",
            "books",
            "bookies",
            "book"
          ],
          "answer": 1
        },
        {
          "q": "Множественное число “box”",
          "options": [
            "boxs",
            "boxies",
            "boxes",
            "box"
          ],
          "answer": 2
        },
        {
          "q": "Множественное число “child”",
          "options": [
            "childs",
            "childes",
            "children",
            "childrens"
          ],
          "answer": 2
        },
        {
          "q": "Выберите: She is ___ engineer.",
          "options": [
            "a",
            "an",
            "two",
            "some"
          ],
          "answer": 1
        },
        {
          "q": "A/an используются перед одним исчисляемым предметом.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Множественное число “person”",
          "options": [
            "persons only",
            "peoples",
            "people",
            "persones"
          ],
          "answer": 2
        }
      ]
    },
    {
      "key": "a1-07",
      "title": "This, That, These, Those",
      "ru": "Указательные местоимения",
      "theory": [
        "This означает «этот/эта/это» для одного предмета рядом. That — «тот/та/то» для одного предмета дальше.",
        "These — множественное число от this: «эти». Those — множественное число от that: «те».",
        "Согласуйте to be: This is ..., That is ..., These are ..., Those are ...."
      ],
      "grammar": [
        [
          "Расстояние",
          "1 предмет",
          "Несколько"
        ],
        [
          "рядом",
          "this + is",
          "these + are"
        ],
        [
          "дальше",
          "that + is",
          "those + are"
        ]
      ],
      "examples": [
        [
          "This is my phone.",
          "Это мой телефон."
        ],
        [
          "That is your bag.",
          "То твоя сумка."
        ],
        [
          "These are my books.",
          "Это мои книги."
        ],
        [
          "Those are their chairs.",
          "Те стулья — их."
        ]
      ],
      "vocab": [
        [
          "this",
          "/ðɪs/",
          "этот; эта; это",
          "This is my pen."
        ],
        [
          "that",
          "/ðæt/",
          "тот; та; то",
          "That is a car."
        ],
        [
          "these",
          "/ðiːz/",
          "эти",
          "These are keys."
        ],
        [
          "those",
          "/ðəʊz/",
          "те",
          "Those are houses."
        ],
        [
          "bag",
          "/bæɡ/",
          "сумка",
          "This is my bag."
        ],
        [
          "key",
          "/kiː/",
          "ключ",
          "These are my keys."
        ]
      ],
      "practice": [
        {
          "q": "Один предмет рядом: ___ is my book.",
          "a": "This",
          "hint": "Один + рядом = this."
        },
        {
          "q": "Несколько предметов далеко: ___ are cars.",
          "a": "Those",
          "hint": "Много + далеко = those."
        },
        {
          "q": "Исправьте: These is my keys.",
          "a": "These are my keys.",
          "hint": "С these используется are."
        }
      ],
      "quiz": [
        {
          "q": "Один предмет рядом: ___ is my phone.",
          "options": [
            "This",
            "These",
            "Those",
            "They"
          ],
          "answer": 0
        },
        {
          "q": "Один предмет далеко: ___ is a bus.",
          "options": [
            "These",
            "That",
            "Those",
            "This are"
          ],
          "answer": 1
        },
        {
          "q": "Несколько предметов рядом: ___ are books.",
          "options": [
            "This",
            "That",
            "These",
            "It"
          ],
          "answer": 2
        },
        {
          "q": "Несколько предметов далеко: ___ are houses.",
          "options": [
            "That",
            "This",
            "Those",
            "These is"
          ],
          "answer": 2
        },
        {
          "q": "Выберите правильное:",
          "options": [
            "These is my keys.",
            "These are my keys.",
            "This are my keys.",
            "Those is my keys."
          ],
          "answer": 1
        },
        {
          "q": "Что означает “those”?",
          "options": [
            "этот",
            "эти рядом",
            "те",
            "тот один"
          ],
          "answer": 2
        },
        {
          "q": "С “this” обычно используется is.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "С “these” используется...",
          "options": [
            "am",
            "is",
            "are",
            "be only"
          ],
          "answer": 2
        }
      ]
    },
    {
      "key": "a1-08",
      "title": "Have Got & Family",
      "ru": "Have got и семья",
      "theory": [
        "Have got означает «иметь». С I/you/we/they используем have got; с he/she/it — has got.",
        "Отрицание: haven’t got / hasn’t got. Вопрос: Have you got ...? Has she got ...? Короткие ответы: Yes, I have. / No, she hasn’t.",
        "Эта конструкция удобна для разговора о семье и принадлежности: I’ve got a brother. She’s got two sisters."
      ],
      "grammar": [
        [
          "Подлежащее",
          "Утверждение",
          "Отрицание",
          "Вопрос"
        ],
        [
          "I/You/We/They",
          "have got",
          "haven’t got",
          "Have ... got?"
        ],
        [
          "He/She/It",
          "has got",
          "hasn’t got",
          "Has ... got?"
        ]
      ],
      "examples": [
        [
          "I’ve got a brother.",
          "У меня есть брат."
        ],
        [
          "She has got two sisters.",
          "У неё две сестры."
        ],
        [
          "Have you got a big family?",
          "У тебя большая семья?"
        ],
        [
          "He hasn’t got a car.",
          "У него нет машины."
        ]
      ],
      "vocab": [
        [
          "family",
          "/ˈfæməli/",
          "семья",
          "I’ve got a big family."
        ],
        [
          "mother",
          "/ˈmʌðə/",
          "мама",
          "My mother is a teacher."
        ],
        [
          "father",
          "/ˈfɑːðə/",
          "папа",
          "My father is an engineer."
        ],
        [
          "brother",
          "/ˈbrʌðə/",
          "брат",
          "I’ve got one brother."
        ],
        [
          "sister",
          "/ˈsɪstə/",
          "сестра",
          "She has got a sister."
        ],
        [
          "parents",
          "/ˈpeərənts/",
          "родители",
          "My parents are at home."
        ],
        [
          "grandmother",
          "/ˈɡrænmʌðə/",
          "бабушка",
          "My grandmother is kind."
        ],
        [
          "grandfather",
          "/ˈɡrænfɑːðə/",
          "дедушка",
          "My grandfather is 70."
        ]
      ],
      "practice": [
        {
          "q": "Дополните: She ___ got a brother.",
          "a": "has",
          "hint": "С she используем has got."
        },
        {
          "q": "Сделайте вопрос: You have got a sister.",
          "a": "Have you got a sister?",
          "hint": "Have переносится в начало."
        },
        {
          "q": "Сделайте отрицание: He has got a car.",
          "a": "He hasn’t got a car.",
          "hint": "has not = hasn’t."
        }
      ],
      "quiz": [
        {
          "q": "Выберите: I ___ got a sister.",
          "options": [
            "has",
            "have",
            "am",
            "is"
          ],
          "answer": 1
        },
        {
          "q": "Выберите: He ___ got two brothers.",
          "options": [
            "have",
            "has",
            "are",
            "do"
          ],
          "answer": 1
        },
        {
          "q": "Правильный вопрос:",
          "options": [
            "Have you got a brother?",
            "Do you got brother?",
            "Has you got a brother?",
            "You have got brother?"
          ],
          "answer": 0
        },
        {
          "q": "Отрицание для she:",
          "options": [
            "she haven’t got",
            "she hasn’t got",
            "she not has got",
            "she don’t got"
          ],
          "answer": 1
        },
        {
          "q": "Что означает “parents”?",
          "options": [
            "дети",
            "родители",
            "братья",
            "друзья"
          ],
          "answer": 1
        },
        {
          "q": "Что означает “sister”?",
          "options": [
            "сестра",
            "мама",
            "бабушка",
            "дочь"
          ],
          "answer": 0
        },
        {
          "q": "С he/she/it используется has got.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Короткий ответ на “Have you got a phone?”",
          "options": [
            "Yes, I have.",
            "Yes, I has.",
            "Yes, I got.",
            "Yes, am."
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-09",
      "title": "Present Simple — Positive & Negative",
      "ru": "Present Simple: утверждения и отрицания",
      "theory": [
        "Present Simple используем для привычек, регулярных действий и фактов: I study every day. We live in Almaty.",
        "С I/you/we/they глагол остаётся в базовой форме. С he/she/it обычно добавляем -s: work → works, play → plays. После -sh/-ch/-ss/-x/-o часто -es: watch → watches, go → goes.",
        "Отрицание: I/you/we/they don’t + глагол; he/she/it doesn’t + глагол. После doesn’t глагол снова без -s: He doesn’t work."
      ],
      "grammar": [
        [
          "Подлежащее",
          "Утверждение",
          "Отрицание"
        ],
        [
          "I/You/We/They",
          "work",
          "don’t work"
        ],
        [
          "He/She/It",
          "works",
          "doesn’t work"
        ],
        [
          "he/she/it + go",
          "goes",
          "doesn’t go"
        ],
        [
          "he/she/it + study",
          "studies",
          "doesn’t study"
        ]
      ],
      "examples": [
        [
          "I study English every day.",
          "Я изучаю английский каждый день."
        ],
        [
          "She works at a school.",
          "Она работает в школе."
        ],
        [
          "They don’t live here.",
          "Они здесь не живут."
        ],
        [
          "He doesn’t play tennis.",
          "Он не играет в теннис."
        ]
      ],
      "vocab": [
        [
          "work",
          "/wɜːk/",
          "работать",
          "I work every day."
        ],
        [
          "study",
          "/ˈstʌdi/",
          "учиться; изучать",
          "She studies English."
        ],
        [
          "live",
          "/lɪv/",
          "жить",
          "We live in Almaty."
        ],
        [
          "play",
          "/pleɪ/",
          "играть",
          "They play football."
        ],
        [
          "watch",
          "/wɒtʃ/",
          "смотреть",
          "He watches TV."
        ],
        [
          "like",
          "/laɪk/",
          "нравиться",
          "I like music."
        ],
        [
          "every day",
          "/ˈevri deɪ/",
          "каждый день",
          "I read every day."
        ]
      ],
      "practice": [
        {
          "q": "Дополните: She ___ English every day. (study)",
          "a": "studies",
          "hint": "С she: study → studies."
        },
        {
          "q": "Сделайте отрицание: He plays football.",
          "a": "He doesn’t play football.",
          "hint": "После doesn’t глагол без -s."
        },
        {
          "q": "Дополните: We ___ in Kazakhstan. (live)",
          "a": "live",
          "hint": "С we базовая форма."
        }
      ],
      "quiz": [
        {
          "q": "Выберите: She ___ English.",
          "options": [
            "study",
            "studies",
            "studys",
            "studying"
          ],
          "answer": 1
        },
        {
          "q": "Выберите: He ___ to school every day.",
          "options": [
            "go",
            "goes",
            "gos",
            "going"
          ],
          "answer": 1
        },
        {
          "q": "Отрицание: I work on Sunday.",
          "options": [
            "I doesn’t work on Sunday.",
            "I don’t work on Sunday.",
            "I not work on Sunday.",
            "I am not work on Sunday."
          ],
          "answer": 1
        },
        {
          "q": "Отрицание: She likes coffee.",
          "options": [
            "She doesn’t like coffee.",
            "She don’t likes coffee.",
            "She doesn’t likes coffee.",
            "She not like coffee."
          ],
          "answer": 0
        },
        {
          "q": "После “doesn’t” основной глагол...",
          "options": [
            "получает -s",
            "остаётся в базовой форме",
            "получает -ing",
            "становится is"
          ],
          "answer": 1
        },
        {
          "q": "Выберите: They ___ football.",
          "options": [
            "plays",
            "play",
            "playes",
            "does play"
          ],
          "answer": 1
        },
        {
          "q": "Present Simple подходит для регулярных действий.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Правильная форма “watch” для he:",
          "options": [
            "watch",
            "watchs",
            "watches",
            "watchies"
          ],
          "answer": 2
        }
      ]
    },
    {
      "key": "a1-10",
      "title": "Present Simple — Questions & Daily Routine",
      "ru": "Present Simple: вопросы и распорядок дня",
      "theory": [
        "Вопросы Present Simple строятся с do/does. С I/you/we/they: Do you work? С he/she/it: Does she work?",
        "После does основной глагол используется без -s: Does he play? — не Does he plays?",
        "Для распорядка дня часто используются выражения get up, have breakfast, go to school/work, have lunch, come home, do homework, go to bed. Время: at seven o’clock, at half past eight."
      ],
      "grammar": [
        [
          "Подлежащее",
          "Вопрос",
          "Короткий ответ"
        ],
        [
          "I/you/we/they",
          "Do ... work?",
          "Yes, ... do. / No, ... don’t."
        ],
        [
          "he/she/it",
          "Does ... work?",
          "Yes, ... does. / No, ... doesn’t."
        ],
        [
          "Вопросительное слово",
          "What/Where/When + do/does ...?",
          "When do you get up?"
        ]
      ],
      "examples": [
        [
          "Do you study English? — Yes, I do.",
          "Ты изучаешь английский? — Да."
        ],
        [
          "Does he work here? — No, he doesn’t.",
          "Он работает здесь? — Нет."
        ],
        [
          "What time do you get up?",
          "Во сколько ты встаёшь?"
        ],
        [
          "She goes to bed at ten o’clock.",
          "Она ложится спать в десять."
        ]
      ],
      "vocab": [
        [
          "get up",
          "/ɡet ʌp/",
          "вставать",
          "I get up at seven."
        ],
        [
          "breakfast",
          "/ˈbrekfəst/",
          "завтрак",
          "I have breakfast at home."
        ],
        [
          "lunch",
          "/lʌntʃ/",
          "обед",
          "We have lunch at one."
        ],
        [
          "homework",
          "/ˈhəʊmwɜːk/",
          "домашнее задание",
          "I do my homework."
        ],
        [
          "go to bed",
          "/ɡəʊ tə bed/",
          "ложиться спать",
          "I go to bed at ten."
        ],
        [
          "usually",
          "/ˈjuːʒuəli/",
          "обычно",
          "I usually get up at seven."
        ],
        [
          "sometimes",
          "/ˈsʌmtaɪmz/",
          "иногда",
          "We sometimes watch TV."
        ],
        [
          "never",
          "/ˈnevə/",
          "никогда",
          "I never drink coffee."
        ]
      ],
      "practice": [
        {
          "q": "Сделайте вопрос: You study English.",
          "a": "Do you study English?",
          "hint": "С you используйте Do."
        },
        {
          "q": "Сделайте вопрос: She works here.",
          "a": "Does she work here?",
          "hint": "После does — work без -s."
        },
        {
          "q": "Как спросить «Во сколько ты встаёшь?»",
          "a": "What time do you get up?",
          "hint": "What time + do + you + get up."
        }
      ],
      "quiz": [
        {
          "q": "Выберите: ___ you study English?",
          "options": [
            "Does",
            "Do",
            "Are",
            "Is"
          ],
          "answer": 1
        },
        {
          "q": "Выберите: ___ she work here?",
          "options": [
            "Do",
            "Does",
            "Is",
            "Has"
          ],
          "answer": 1
        },
        {
          "q": "Правильный вопрос:",
          "options": [
            "Does he plays football?",
            "Does he play football?",
            "Do he play football?",
            "He does play football?"
          ],
          "answer": 1
        },
        {
          "q": "Короткий ответ на “Do they live here?”",
          "options": [
            "Yes, they are.",
            "Yes, they do.",
            "Yes, they does.",
            "Yes, they live do."
          ],
          "answer": 1
        },
        {
          "q": "Что означает “get up”?",
          "options": [
            "ложиться спать",
            "вставать",
            "завтракать",
            "идти домой"
          ],
          "answer": 1
        },
        {
          "q": "Что означает “go to bed”?",
          "options": [
            "идти в школу",
            "ложиться спать",
            "вставать",
            "делать уроки"
          ],
          "answer": 1
        },
        {
          "q": "После “Does she ...?” основной глагол используется без -s.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Как спросить «Во сколько ты встаёшь?»",
          "options": [
            "What time do you get up?",
            "When you are get up?",
            "What time does you get up?",
            "How old do you get up?"
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-11",
      "title": "Days, Months & Dates",
      "ru": "Дни недели, месяцы и даты",
      "theory": [
        "Дни недели в английском пишутся с заглавной буквы: Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday.",
        "Месяцы также пишутся с заглавной буквы: January, February, March и т.д. Для дат обычно нужны порядковые числительные: first, second, third, fourth, fifth.",
        "Чтобы спросить дату, используйте What’s the date today? Чтобы спросить день недели: What day is it today?"
      ],
      "grammar": [
        [
          "Вопрос",
          "Ответ",
          "Перевод"
        ],
        [
          "What day is it today?",
          "It’s Monday.",
          "Какой сегодня день?"
        ],
        [
          "What’s the date today?",
          "It’s the fifth of May.",
          "Какое сегодня число?"
        ],
        [
          "When is your birthday?",
          "It’s in July.",
          "Когда твой день рождения?"
        ]
      ],
      "examples": [
        [
          "Today is Monday.",
          "Сегодня понедельник."
        ],
        [
          "My birthday is in September.",
          "Мой день рождения в сентябре."
        ],
        [
          "It’s the first of January.",
          "Первое января."
        ],
        [
          "School starts in September.",
          "Школа начинается в сентябре."
        ]
      ],
      "vocab": [
        [
          "Monday",
          "/ˈmʌndeɪ/",
          "понедельник",
          "Today is Monday."
        ],
        [
          "Tuesday",
          "/ˈtjuːzdeɪ/",
          "вторник",
          "See you on Tuesday."
        ],
        [
          "Wednesday",
          "/ˈwenzdeɪ/",
          "среда",
          "We study on Wednesday."
        ],
        [
          "Friday",
          "/ˈfraɪdeɪ/",
          "пятница",
          "Friday is my favourite day."
        ],
        [
          "weekend",
          "/ˌwiːkˈend/",
          "выходные",
          "I relax at the weekend."
        ],
        [
          "month",
          "/mʌnθ/",
          "месяц",
          "There are twelve months."
        ],
        [
          "birthday",
          "/ˈbɜːθdeɪ/",
          "день рождения",
          "My birthday is in May."
        ]
      ],
      "practice": [
        {
          "q": "Как сказать «Сегодня пятница»?",
          "a": "Today is Friday.",
          "hint": "Дни недели пишутся с заглавной буквы."
        },
        {
          "q": "Как спросить «Когда твой день рождения?»",
          "a": "When is your birthday?",
          "hint": "Начните с When is..."
        },
        {
          "q": "Напишите по-английски «январь».",
          "a": "January",
          "hint": "Первый месяц года."
        }
      ],
      "quiz": [
        {
          "q": "Какой день идёт после Monday?",
          "options": [
            "Sunday",
            "Tuesday",
            "Friday",
            "Thursday"
          ],
          "answer": 1
        },
        {
          "q": "Что означает “weekend”?",
          "options": [
            "рабочий день",
            "месяц",
            "выходные",
            "год"
          ],
          "answer": 2
        },
        {
          "q": "Как спросить «Какой сегодня день?»",
          "options": [
            "What day is it today?",
            "What date are you?",
            "When day today?",
            "How day is it?"
          ],
          "answer": 0
        },
        {
          "q": "Месяцы в английском пишутся...",
          "options": [
            "только маленькой буквой",
            "с заглавной буквы",
            "цифрами",
            "без пробелов"
          ],
          "answer": 1
        },
        {
          "q": "Как сказать «Мой день рождения в июле»?",
          "options": [
            "My birthday is on July.",
            "My birthday is in July.",
            "My birthday at July.",
            "I birthday July."
          ],
          "answer": 1
        },
        {
          "q": "Сколько месяцев в году?",
          "options": [
            "10",
            "11",
            "12",
            "13"
          ],
          "answer": 2
        },
        {
          "q": "“Wednesday” — это среда.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Как спросить дату?",
          "options": [
            "What’s the date today?",
            "What’s your weekday?",
            "How date is?",
            "Where is date?"
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-12",
      "title": "Telling the Time",
      "ru": "Как говорить время",
      "theory": [
        "Чтобы спросить время, используйте What time is it? или What’s the time? Ответ: It’s seven o’clock.",
        "Для половины часа используется half past: 7:30 — half past seven. Для четверти часа: 7:15 — quarter past seven, 7:45 — quarter to eight.",
        "Для расписания используйте предлог at: I get up at seven o’clock."
      ],
      "grammar": [
        [
          "Время",
          "Фраза",
          "Перевод"
        ],
        [
          "7:00",
          "seven o’clock",
          "семь часов"
        ],
        [
          "7:15",
          "quarter past seven",
          "четверть восьмого"
        ],
        [
          "7:30",
          "half past seven",
          "половина восьмого"
        ],
        [
          "7:45",
          "quarter to eight",
          "без четверти восемь"
        ]
      ],
      "examples": [
        [
          "What time is it?",
          "Который час?"
        ],
        [
          "It’s half past eight.",
          "Сейчас половина девятого."
        ],
        [
          "I go to school at eight o’clock.",
          "Я иду в школу в восемь."
        ],
        [
          "The lesson starts at quarter past nine.",
          "Урок начинается в 9:15."
        ]
      ],
      "vocab": [
        [
          "o’clock",
          "/əˈklɒk/",
          "ровно (о времени)",
          "It’s five o’clock."
        ],
        [
          "half",
          "/hɑːf/",
          "половина",
          "Half past six."
        ],
        [
          "quarter",
          "/ˈkwɔːtə/",
          "четверть",
          "Quarter to ten."
        ],
        [
          "past",
          "/pɑːst/",
          "после",
          "Ten past three."
        ],
        [
          "to",
          "/tuː/",
          "до",
          "Ten to four."
        ],
        [
          "start",
          "/stɑːt/",
          "начинаться",
          "School starts at eight."
        ]
      ],
      "practice": [
        {
          "q": "Как сказать 6:30?",
          "a": "half past six",
          "hint": "30 минут = half past."
        },
        {
          "q": "Как сказать 8:15?",
          "a": "quarter past eight",
          "hint": "15 минут после часа."
        },
        {
          "q": "Как спросить время?",
          "a": "What time is it?",
          "hint": "What time..."
        }
      ],
      "quiz": [
        {
          "q": "7:00 =",
          "options": [
            "seven o’clock",
            "half seven",
            "seven past",
            "quarter seven"
          ],
          "answer": 0
        },
        {
          "q": "6:30 =",
          "options": [
            "quarter to six",
            "half past six",
            "six o’clock",
            "half to six"
          ],
          "answer": 1
        },
        {
          "q": "9:15 =",
          "options": [
            "quarter to nine",
            "quarter past nine",
            "half past nine",
            "nine to quarter"
          ],
          "answer": 1
        },
        {
          "q": "7:45 =",
          "options": [
            "quarter past seven",
            "quarter to eight",
            "half past seven",
            "seven o’clock"
          ],
          "answer": 1
        },
        {
          "q": "Как спросить время?",
          "options": [
            "What time is it?",
            "How time are you?",
            "Where time?",
            "When clock?"
          ],
          "answer": 0
        },
        {
          "q": "Перед временем в расписании обычно используется...",
          "options": [
            "in",
            "on",
            "at",
            "from"
          ],
          "answer": 2
        },
        {
          "q": "“past” используется для минут после часа.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Что означает “start”?",
          "options": [
            "заканчивать",
            "начинать(ся)",
            "опаздывать",
            "спать"
          ],
          "answer": 1
        }
      ]
    },
    {
      "key": "a1-13",
      "title": "Prepositions of Time & Place",
      "ru": "Предлоги времени и места",
      "theory": [
        "Для времени: at — точное время (at 7 o’clock), on — дни и даты (on Monday), in — месяцы, годы и части дня (in May, in 2026, in the morning).",
        "Для места: in — внутри, on — на поверхности, under — под, next to — рядом, between — между, behind — позади, in front of — перед.",
        "Эти предлоги лучше запоминать в коротких готовых фразах."
      ],
      "grammar": [
        [
          "Предлог",
          "Использование",
          "Пример"
        ],
        [
          "at",
          "точное время",
          "at 8 o’clock"
        ],
        [
          "on",
          "день/дата",
          "on Friday"
        ],
        [
          "in",
          "месяц/год/внутри",
          "in June; in the box"
        ],
        [
          "under",
          "под",
          "under the table"
        ],
        [
          "next to",
          "рядом",
          "next to the school"
        ]
      ],
      "examples": [
        [
          "The book is on the table.",
          "Книга на столе."
        ],
        [
          "The cat is under the chair.",
          "Кошка под стулом."
        ],
        [
          "I study English on Monday.",
          "Я учу английский в понедельник."
        ],
        [
          "My birthday is in October.",
          "Мой день рождения в октябре."
        ]
      ],
      "vocab": [
        [
          "under",
          "/ˈʌndə/",
          "под",
          "The bag is under the desk."
        ],
        [
          "between",
          "/bɪˈtwiːn/",
          "между",
          "The bank is between two shops."
        ],
        [
          "behind",
          "/bɪˈhaɪnd/",
          "позади",
          "The car is behind the house."
        ],
        [
          "next to",
          "/nekst tuː/",
          "рядом с",
          "The cafe is next to the school."
        ],
        [
          "in front of",
          "/ɪn frʌnt əv/",
          "перед",
          "He is in front of the door."
        ]
      ],
      "practice": [
        {
          "q": "Выберите предлог: ___ Monday.",
          "a": "on",
          "hint": "День недели → on."
        },
        {
          "q": "Выберите предлог: ___ 8 o’clock.",
          "a": "at",
          "hint": "Точное время → at."
        },
        {
          "q": "Переведите: «под столом».",
          "a": "under the table",
          "hint": "Под = under."
        }
      ],
      "quiz": [
        {
          "q": "___ Monday",
          "options": [
            "at",
            "on",
            "in",
            "under"
          ],
          "answer": 1
        },
        {
          "q": "___ July",
          "options": [
            "in",
            "on",
            "at",
            "next"
          ],
          "answer": 0
        },
        {
          "q": "___ 9 o’clock",
          "options": [
            "on",
            "in",
            "at",
            "between"
          ],
          "answer": 2
        },
        {
          "q": "“under the table” означает...",
          "options": [
            "на столе",
            "под столом",
            "рядом со столом",
            "перед столом"
          ],
          "answer": 1
        },
        {
          "q": "“next to” означает...",
          "options": [
            "между",
            "за",
            "рядом с",
            "внутри"
          ],
          "answer": 2
        },
        {
          "q": "Книга на столе: The book is ___ the table.",
          "options": [
            "under",
            "on",
            "between",
            "in front"
          ],
          "answer": 1
        },
        {
          "q": "Для месяцев обычно используем in.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "“between” означает...",
          "options": [
            "между",
            "над",
            "под",
            "рядом"
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-14",
      "title": "There Is / There Are",
      "ru": "Конструкция there is / there are",
      "theory": [
        "There is используется, когда говорим об одном предмете: There is a table in the room. There are — о нескольких: There are two chairs.",
        "Отрицание: There isn’t ... / There aren’t .... Вопрос: Is there ...? / Are there ...?",
        "С some часто говорим о неопределённом количестве во множественном числе: There are some books."
      ],
      "grammar": [
        [
          "Количество",
          "Утверждение",
          "Отрицание",
          "Вопрос"
        ],
        [
          "1",
          "There is",
          "There isn’t",
          "Is there...?"
        ],
        [
          "2+",
          "There are",
          "There aren’t",
          "Are there...?"
        ]
      ],
      "examples": [
        [
          "There is a sofa in the room.",
          "В комнате есть диван."
        ],
        [
          "There are three windows.",
          "Есть три окна."
        ],
        [
          "Is there a bathroom?",
          "Есть ванная?"
        ],
        [
          "There aren’t any chairs.",
          "Стульев нет."
        ]
      ],
      "vocab": [
        [
          "room",
          "/ruːm/",
          "комната",
          "There is a room."
        ],
        [
          "window",
          "/ˈwɪndəʊ/",
          "окно",
          "There are two windows."
        ],
        [
          "door",
          "/dɔː/",
          "дверь",
          "The door is open."
        ],
        [
          "sofa",
          "/ˈsəʊfə/",
          "диван",
          "There is a sofa."
        ],
        [
          "kitchen",
          "/ˈkɪtʃɪn/",
          "кухня",
          "The kitchen is small."
        ],
        [
          "bathroom",
          "/ˈbɑːθruːm/",
          "ванная",
          "Is there a bathroom?"
        ]
      ],
      "practice": [
        {
          "q": "Дополните: There ___ a table.",
          "a": "is",
          "hint": "Один предмет → is."
        },
        {
          "q": "Дополните: There ___ two chairs.",
          "a": "are",
          "hint": "Несколько → are."
        },
        {
          "q": "Сделайте вопрос: There is a kitchen.",
          "a": "Is there a kitchen?",
          "hint": "Is переносится в начало."
        }
      ],
      "quiz": [
        {
          "q": "There ___ a bed in the room.",
          "options": [
            "are",
            "is",
            "be",
            "am"
          ],
          "answer": 1
        },
        {
          "q": "There ___ three windows.",
          "options": [
            "is",
            "are",
            "has",
            "be"
          ],
          "answer": 1
        },
        {
          "q": "Вопрос к “There is a bathroom.”",
          "options": [
            "Is there a bathroom?",
            "Does there bathroom?",
            "Are there a bathroom?",
            "There is bathroom?"
          ],
          "answer": 0
        },
        {
          "q": "Отрицание для множественного числа:",
          "options": [
            "There isn’t chairs.",
            "There aren’t chairs.",
            "There not are chairs.",
            "There doesn’t chairs."
          ],
          "answer": 1
        },
        {
          "q": "Что означает “kitchen”?",
          "options": [
            "спальня",
            "ванная",
            "кухня",
            "окно"
          ],
          "answer": 2
        },
        {
          "q": "Что означает “window”?",
          "options": [
            "окно",
            "дверь",
            "стул",
            "стол"
          ],
          "answer": 0
        },
        {
          "q": "There are используется с несколькими предметами.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Правильно:",
          "options": [
            "There are a sofa.",
            "There is a sofa.",
            "There am sofa.",
            "There be a sofa."
          ],
          "answer": 1
        }
      ]
    },
    {
      "key": "a1-15",
      "title": "Food, Some & Any",
      "ru": "Еда, some и any",
      "theory": [
        "Some и any используются с множественными исчисляемыми и неисчисляемыми существительными. Some обычно в утверждениях: I’ve got some apples. Any часто в вопросах и отрицаниях: Have you got any milk? I haven’t got any milk.",
        "Неисчисляемые существительные обычно не имеют формы множественного числа: water, milk, bread, rice, cheese.",
        "В кафе полезны фразы: I’d like ..., Can I have ...?, How much is it?"
      ],
      "grammar": [
        [
          "Тип",
          "Обычно",
          "Пример"
        ],
        [
          "Утверждение",
          "some",
          "There is some water."
        ],
        [
          "Вопрос",
          "any",
          "Is there any milk?"
        ],
        [
          "Отрицание",
          "any",
          "There isn’t any bread."
        ]
      ],
      "examples": [
        [
          "I’d like some water, please.",
          "Я хотел(а) бы воды, пожалуйста."
        ],
        [
          "Have you got any apples?",
          "У тебя есть яблоки?"
        ],
        [
          "There isn’t any milk.",
          "Молока нет."
        ],
        [
          "We have some bread and cheese.",
          "У нас есть хлеб и сыр."
        ]
      ],
      "vocab": [
        [
          "bread",
          "/bred/",
          "хлеб",
          "I eat bread for breakfast."
        ],
        [
          "milk",
          "/mɪlk/",
          "молоко",
          "Do you drink milk?"
        ],
        [
          "cheese",
          "/tʃiːz/",
          "сыр",
          "I like cheese."
        ],
        [
          "rice",
          "/raɪs/",
          "рис",
          "We have some rice."
        ],
        [
          "water",
          "/ˈwɔːtə/",
          "вода",
          "I’d like some water."
        ],
        [
          "apple",
          "/ˈæpəl/",
          "яблоко",
          "There are some apples."
        ],
        [
          "hungry",
          "/ˈhʌŋɡri/",
          "голодный",
          "I’m hungry."
        ],
        [
          "thirsty",
          "/ˈθɜːsti/",
          "хочу пить",
          "I’m thirsty."
        ]
      ],
      "practice": [
        {
          "q": "Дополните: I have ___ apples.",
          "a": "some",
          "hint": "Утверждение → some."
        },
        {
          "q": "Дополните: Have you got ___ milk?",
          "a": "any",
          "hint": "Вопрос → any."
        },
        {
          "q": "Как сказать «Я хотел бы воды»?",
          "a": "I’d like some water.",
          "hint": "I’d like..."
        }
      ],
      "quiz": [
        {
          "q": "I have ___ bread.",
          "options": [
            "any",
            "some",
            "an",
            "manys"
          ],
          "answer": 1
        },
        {
          "q": "Have you got ___ milk?",
          "options": [
            "some",
            "any",
            "a",
            "two"
          ],
          "answer": 1
        },
        {
          "q": "There isn’t ___ cheese.",
          "options": [
            "some",
            "any",
            "an",
            "many"
          ],
          "answer": 1
        },
        {
          "q": "Что означает “hungry”?",
          "options": [
            "хочу пить",
            "голодный",
            "уставший",
            "счастливый"
          ],
          "answer": 1
        },
        {
          "q": "Что означает “thirsty”?",
          "options": [
            "голодный",
            "хочу пить",
            "холодный",
            "занятый"
          ],
          "answer": 1
        },
        {
          "q": "Как вежливо заказать воду?",
          "options": [
            "Give water.",
            "I’d like some water, please.",
            "Water now.",
            "I am water."
          ],
          "answer": 1
        },
        {
          "q": "Some чаще используется в утверждениях.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Bread обычно неисчисляемое существительное.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-16",
      "title": "Can / Can’t",
      "ru": "Умения и возможности: can / can’t",
      "theory": [
        "Can означает «мочь/уметь». После can всегда идёт базовая форма глагола без to: I can swim. She can speak English.",
        "Отрицание: cannot или can’t. Вопрос: Can you swim? Короткие ответы: Yes, I can. / No, I can’t.",
        "Can также используют для простых просьб: Can you help me?"
      ],
      "grammar": [
        [
          "Тип",
          "Схема",
          "Пример"
        ],
        [
          "Утверждение",
          "subject + can + verb",
          "He can drive."
        ],
        [
          "Отрицание",
          "subject + can’t + verb",
          "He can’t drive."
        ],
        [
          "Вопрос",
          "Can + subject + verb?",
          "Can he drive?"
        ]
      ],
      "examples": [
        [
          "I can swim.",
          "Я умею плавать."
        ],
        [
          "She can’t drive.",
          "Она не умеет водить."
        ],
        [
          "Can you speak English?",
          "Ты умеешь говорить по-английски?"
        ],
        [
          "Can you help me?",
          "Ты можешь мне помочь?"
        ]
      ],
      "vocab": [
        [
          "swim",
          "/swɪm/",
          "плавать",
          "I can swim."
        ],
        [
          "drive",
          "/draɪv/",
          "водить",
          "He can drive."
        ],
        [
          "cook",
          "/kʊk/",
          "готовить",
          "She can cook."
        ],
        [
          "dance",
          "/dɑːns/",
          "танцевать",
          "They can dance."
        ],
        [
          "sing",
          "/sɪŋ/",
          "петь",
          "Can you sing?"
        ],
        [
          "help",
          "/help/",
          "помогать",
          "Can you help me?"
        ]
      ],
      "practice": [
        {
          "q": "Скажите: «Я умею плавать».",
          "a": "I can swim.",
          "hint": "can + базовая форма."
        },
        {
          "q": "Сделайте отрицание: She can drive.",
          "a": "She can’t drive.",
          "hint": "can’t + verb."
        },
        {
          "q": "Сделайте вопрос: You can cook.",
          "a": "Can you cook?",
          "hint": "Can в начало."
        }
      ],
      "quiz": [
        {
          "q": "Выберите: I can ___ .",
          "options": [
            "to swim",
            "swim",
            "swims",
            "swimming"
          ],
          "answer": 1
        },
        {
          "q": "Отрицание can:",
          "options": [
            "don’t can",
            "can’t",
            "doesn’t can",
            "not can to"
          ],
          "answer": 1
        },
        {
          "q": "Правильный вопрос:",
          "options": [
            "Do you can swim?",
            "Can you swim?",
            "Can you to swim?",
            "You can swim?"
          ],
          "answer": 1
        },
        {
          "q": "Короткий ответ: Can she cook?",
          "options": [
            "Yes, she does.",
            "Yes, she can.",
            "Yes, she is.",
            "Yes, can she."
          ],
          "answer": 1
        },
        {
          "q": "После can используется...",
          "options": [
            "глагол с -s",
            "to + глагол",
            "базовая форма глагола",
            "-ing"
          ],
          "answer": 2
        },
        {
          "q": "Что означает “drive”?",
          "options": [
            "бегать",
            "водить",
            "готовить",
            "рисовать"
          ],
          "answer": 1
        },
        {
          "q": "Can можно использовать для просьб.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "“I can’t sing” означает...",
          "options": [
            "Я люблю петь.",
            "Я не умею петь.",
            "Я сейчас пою.",
            "Я буду петь."
          ],
          "answer": 1
        }
      ]
    },
    {
      "key": "a1-17",
      "title": "Present Continuous",
      "ru": "Present Continuous: действия сейчас",
      "theory": [
        "Present Continuous используется для действий, происходящих прямо сейчас: I am reading now. She is working at the moment.",
        "Формула: am/is/are + глагол-ing. I am studying, he is studying, they are studying.",
        "Отрицание: isn’t/aren’t + -ing. Вопрос: Are you studying? Is she sleeping?"
      ],
      "grammar": [
        [
          "Подлежащее",
          "Утверждение",
          "Отрицание"
        ],
        [
          "I",
          "am working",
          "am not working"
        ],
        [
          "He/She/It",
          "is working",
          "isn’t working"
        ],
        [
          "You/We/They",
          "are working",
          "aren’t working"
        ]
      ],
      "examples": [
        [
          "I’m reading now.",
          "Я сейчас читаю."
        ],
        [
          "She is talking on the phone.",
          "Она разговаривает по телефону."
        ],
        [
          "They aren’t sleeping.",
          "Они не спят."
        ],
        [
          "Are you listening to me?",
          "Ты меня слушаешь?"
        ]
      ],
      "vocab": [
        [
          "now",
          "/naʊ/",
          "сейчас",
          "I’m working now."
        ],
        [
          "read",
          "/riːd/",
          "читать",
          "She is reading."
        ],
        [
          "write",
          "/raɪt/",
          "писать",
          "He is writing."
        ],
        [
          "listen",
          "/ˈlɪsən/",
          "слушать",
          "We are listening."
        ],
        [
          "sleep",
          "/sliːp/",
          "спать",
          "The baby is sleeping."
        ],
        [
          "talk",
          "/tɔːk/",
          "разговаривать",
          "They are talking."
        ]
      ],
      "practice": [
        {
          "q": "Дополните: I ___ reading now.",
          "a": "am",
          "hint": "I + am."
        },
        {
          "q": "Дополните: She is ___ . (work)",
          "a": "working",
          "hint": "work + ing."
        },
        {
          "q": "Сделайте вопрос: They are studying.",
          "a": "Are they studying?",
          "hint": "Are в начало."
        }
      ],
      "quiz": [
        {
          "q": "I ___ reading now.",
          "options": [
            "is",
            "are",
            "am",
            "do"
          ],
          "answer": 2
        },
        {
          "q": "She is ___ now.",
          "options": [
            "work",
            "works",
            "working",
            "to work"
          ],
          "answer": 2
        },
        {
          "q": "They ___ sleeping.",
          "options": [
            "is",
            "are",
            "am",
            "does"
          ],
          "answer": 1
        },
        {
          "q": "Вопрос: You are listening.",
          "options": [
            "Do you listening?",
            "Are you listening?",
            "Is you listening?",
            "You are listening?"
          ],
          "answer": 1
        },
        {
          "q": "Present Continuous =",
          "options": [
            "am/is/are + verb-ing",
            "do/does + verb",
            "can + verb",
            "have got"
          ],
          "answer": 0
        },
        {
          "q": "Что означает “now”?",
          "options": [
            "вчера",
            "обычно",
            "сейчас",
            "завтра"
          ],
          "answer": 2
        },
        {
          "q": "“She isn’t sleeping” — правильное отрицание.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Что означает “listen”?",
          "options": [
            "смотреть",
            "слушать",
            "говорить",
            "писать"
          ],
          "answer": 1
        }
      ]
    },
    {
      "key": "a1-18",
      "title": "Present Simple vs Present Continuous",
      "ru": "Present Simple и Present Continuous",
      "theory": [
        "Present Simple описывает привычки и регулярные действия: I play football every Friday. Present Continuous — действие сейчас: I am playing football now.",
        "Подсказки Present Simple: every day, usually, often, sometimes, never. Подсказки Present Continuous: now, right now, at the moment.",
        "Важно смотреть не только на слово-подсказку, но и на смысл предложения."
      ],
      "grammar": [
        [
          "Время",
          "Когда используем",
          "Пример"
        ],
        [
          "Present Simple",
          "привычка/факт",
          "She works every day."
        ],
        [
          "Present Continuous",
          "сейчас",
          "She is working now."
        ]
      ],
      "examples": [
        [
          "I usually walk to school.",
          "Я обычно хожу в школу пешком."
        ],
        [
          "Today I am going by bus.",
          "Сегодня я еду на автобусе."
        ],
        [
          "He plays tennis on Saturdays.",
          "Он играет в теннис по субботам."
        ],
        [
          "He is playing tennis now.",
          "Он сейчас играет в теннис."
        ]
      ],
      "vocab": [
        [
          "usually",
          "/ˈjuːʒuəli/",
          "обычно",
          "I usually get up at seven."
        ],
        [
          "often",
          "/ˈɒfən/",
          "часто",
          "We often watch films."
        ],
        [
          "sometimes",
          "/ˈsʌmtaɪmz/",
          "иногда",
          "She sometimes walks."
        ],
        [
          "at the moment",
          "/ət ðə ˈməʊmənt/",
          "в данный момент",
          "He is studying at the moment."
        ],
        [
          "today",
          "/təˈdeɪ/",
          "сегодня",
          "Today I’m staying home."
        ]
      ],
      "practice": [
        {
          "q": "Выберите форму: He ___ every day. (work)",
          "a": "works",
          "hint": "Every day → Present Simple."
        },
        {
          "q": "Выберите форму: He ___ now. (work)",
          "a": "is working",
          "hint": "Now → Present Continuous."
        },
        {
          "q": "Какое время подходит для привычек?",
          "a": "Present Simple",
          "hint": "Регулярные действия."
        }
      ],
      "quiz": [
        {
          "q": "She ___ every day.",
          "options": [
            "is working",
            "works",
            "work now",
            "working"
          ],
          "answer": 1
        },
        {
          "q": "She ___ now.",
          "options": [
            "works",
            "work",
            "is working",
            "does work now"
          ],
          "answer": 2
        },
        {
          "q": "“usually” чаще связано с...",
          "options": [
            "Present Simple",
            "Present Continuous",
            "Past Simple",
            "can"
          ],
          "answer": 0
        },
        {
          "q": "“at the moment” чаще связано с...",
          "options": [
            "Present Simple",
            "Present Continuous",
            "have got",
            "to be only"
          ],
          "answer": 1
        },
        {
          "q": "I ___ football every Friday.",
          "options": [
            "am playing",
            "play",
            "plays",
            "is play"
          ],
          "answer": 1
        },
        {
          "q": "I ___ football right now.",
          "options": [
            "play",
            "plays",
            "am playing",
            "do play"
          ],
          "answer": 2
        },
        {
          "q": "Present Simple подходит для привычек.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Present Continuous подходит для действий прямо сейчас.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-19",
      "title": "Clothes & Shopping",
      "ru": "Одежда и покупки",
      "theory": [
        "Для покупок полезны фразы: How much is this? — Сколько это стоит? How much are these? — Сколько стоят эти? Can I try it on? — Можно примерить?",
        "Для одного предмета используйте it/this, для нескольких — they/these. Размер можно спросить: What size are you? или Do you have this in size M?",
        "Чтобы выразить желание купить: I’d like this T-shirt, please."
      ],
      "grammar": [
        [
          "Ситуация",
          "Фраза"
        ],
        [
          "Цена одного",
          "How much is this?"
        ],
        [
          "Цена нескольких",
          "How much are these?"
        ],
        [
          "Примерка",
          "Can I try it on?"
        ],
        [
          "Размер",
          "Do you have this in size M?"
        ]
      ],
      "examples": [
        [
          "How much is this jacket?",
          "Сколько стоит эта куртка?"
        ],
        [
          "Can I try it on?",
          "Можно это примерить?"
        ],
        [
          "These shoes are too big.",
          "Эти туфли слишком большие."
        ],
        [
          "I’d like this T-shirt, please.",
          "Я хотел(а) бы эту футболку."
        ]
      ],
      "vocab": [
        [
          "T-shirt",
          "/ˈtiː ʃɜːt/",
          "футболка",
          "This T-shirt is nice."
        ],
        [
          "jacket",
          "/ˈdʒækɪt/",
          "куртка",
          "I need a jacket."
        ],
        [
          "trousers",
          "/ˈtraʊzəz/",
          "брюки",
          "These trousers are black."
        ],
        [
          "shoes",
          "/ʃuːz/",
          "обувь/туфли",
          "These shoes are new."
        ],
        [
          "size",
          "/saɪz/",
          "размер",
          "What size do you need?"
        ],
        [
          "cheap",
          "/tʃiːp/",
          "дешёвый",
          "It’s cheap."
        ],
        [
          "expensive",
          "/ɪkˈspensɪv/",
          "дорогой",
          "That jacket is expensive."
        ]
      ],
      "practice": [
        {
          "q": "Как спросить цену одного предмета?",
          "a": "How much is this?",
          "hint": "Один → is."
        },
        {
          "q": "Как спросить «Можно примерить?»",
          "a": "Can I try it on?",
          "hint": "Can I..."
        },
        {
          "q": "Переведите «Эта куртка дорогая».",
          "a": "This jacket is expensive.",
          "hint": "expensive = дорогой."
        }
      ],
      "quiz": [
        {
          "q": "Как спросить цену одного предмета?",
          "options": [
            "How much is this?",
            "How many is this?",
            "What cost these?",
            "How price it?"
          ],
          "answer": 0
        },
        {
          "q": "Как спросить цену нескольких вещей?",
          "options": [
            "How much are these?",
            "How much is these?",
            "How many are price?",
            "What these cost is?"
          ],
          "answer": 0
        },
        {
          "q": "“Can I try it on?” означает...",
          "options": [
            "Можно оплатить?",
            "Можно примерить?",
            "Можно вернуть?",
            "Есть другой цвет?"
          ],
          "answer": 1
        },
        {
          "q": "Что означает “size”?",
          "options": [
            "цена",
            "размер",
            "цвет",
            "касса"
          ],
          "answer": 1
        },
        {
          "q": "Что означает “cheap”?",
          "options": [
            "дорогой",
            "дешёвый",
            "маленький",
            "новый"
          ],
          "answer": 1
        },
        {
          "q": "Что означает “expensive”?",
          "options": [
            "дешёвый",
            "дорогой",
            "красивый",
            "старый"
          ],
          "answer": 1
        },
        {
          "q": "“These shoes” — несколько предметов.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        },
        {
          "q": "Как сказать «Я хотел бы эту футболку»?",
          "options": [
            "I’d like this T-shirt.",
            "I like this T-shirt yesterday.",
            "Give T-shirt.",
            "I am T-shirt."
          ],
          "answer": 0
        }
      ]
    },
    {
      "key": "a1-20",
      "title": "A1 Review 1",
      "ru": "Повторение уроков 1–19",
      "theory": [
        "Этот урок повторяет ключевые темы первой большой части A1: to be, have got, Present Simple, Present Continuous, can/can’t, there is/there are, some/any, время и предлоги.",
        "Перед тестом вернитесь к тем урокам, где были ошибки. Цель — не просто запомнить ответы, а понять, почему используется конкретная форма.",
        "После успешного прохождения этого урока будет открыт следующий блок A1."
      ],
      "grammar": [
        [
          "Тема",
          "Ключевая схема"
        ],
        [
          "to be",
          "I am; he/she is; we/you/they are"
        ],
        [
          "Present Simple",
          "I work; she works; don’t/doesn’t"
        ],
        [
          "Present Continuous",
          "am/is/are + verb-ing"
        ],
        [
          "can",
          "can + base verb"
        ],
        [
          "there is/are",
          "1 предмет / несколько"
        ]
      ],
      "examples": [
        [
          "She is a student and she studies English.",
          "Она ученица и изучает английский."
        ],
        [
          "There are some books on the table.",
          "На столе есть несколько книг."
        ],
        [
          "He can swim but he can’t drive.",
          "Он умеет плавать, но не умеет водить."
        ],
        [
          "They are watching TV now.",
          "Они сейчас смотрят телевизор."
        ]
      ],
      "vocab": [
        [
          "review",
          "/rɪˈvjuː/",
          "повторение",
          "This is a review lesson."
        ],
        [
          "remember",
          "/rɪˈmembə/",
          "помнить",
          "Remember the rule."
        ],
        [
          "mistake",
          "/mɪˈsteɪk/",
          "ошибка",
          "Check your mistakes."
        ],
        [
          "correct",
          "/kəˈrekt/",
          "правильный",
          "Choose the correct answer."
        ],
        [
          "ready",
          "/ˈredi/",
          "готовый",
          "Are you ready?"
        ]
      ],
      "practice": [
        {
          "q": "She ___ English every day. (study)",
          "a": "studies",
          "hint": "Present Simple, she."
        },
        {
          "q": "They ___ watching TV now.",
          "a": "are",
          "hint": "Present Continuous."
        },
        {
          "q": "There ___ two chairs.",
          "a": "are",
          "hint": "Несколько предметов."
        }
      ],
      "quiz": [
        {
          "q": "She ___ a teacher.",
          "options": [
            "am",
            "is",
            "are",
            "be"
          ],
          "answer": 1
        },
        {
          "q": "He ___ English every day.",
          "options": [
            "study",
            "studies",
            "is study",
            "studying"
          ],
          "answer": 1
        },
        {
          "q": "They ___ football now.",
          "options": [
            "play",
            "plays",
            "are playing",
            "is playing"
          ],
          "answer": 2
        },
        {
          "q": "I can ___ .",
          "options": [
            "to swim",
            "swim",
            "swims",
            "swimming"
          ],
          "answer": 1
        },
        {
          "q": "There ___ two windows.",
          "options": [
            "is",
            "are",
            "am",
            "has"
          ],
          "answer": 1
        },
        {
          "q": "Have you got ___ milk?",
          "options": [
            "some",
            "any",
            "a",
            "manys"
          ],
          "answer": 1
        },
        {
          "q": "___ Monday",
          "options": [
            "at",
            "on",
            "in",
            "under"
          ],
          "answer": 1
        },
        {
          "q": "7:30 =",
          "options": [
            "quarter past seven",
            "half past seven",
            "quarter to seven",
            "seven o’clock"
          ],
          "answer": 1
        },
        {
          "q": "How much ___ these shoes?",
          "options": [
            "is",
            "are",
            "am",
            "does"
          ],
          "answer": 1
        },
        {
          "q": "Следующий урок открывается после результата не ниже 80%.",
          "options": [
            "Да",
            "Нет"
          ],
          "answer": 0
        }
      ]
    }
  ]
};
