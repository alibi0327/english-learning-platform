...
window.openStudentStatus = (id) => {
  ...
  const levels = ['A1','A2','KZ-A1','KZ-A2','KZ-B1','KZ-B2','KZ-C1']
  ...
  document.getElementById('studentCourseStatus').innerHTML =
    levels || '<p class="muted">У ученика пока нет прогресса по доступным уровням.</p>';
...
function courseChecks(targetId, selected = new Set()) {
  ...
  const group = (title, items) => `
    <section class="course-access-language">
      <div class="course-access-language-title">${title}</div>
      <div class="course-access-levels">
        ${items.map(c => `
          <label class="course-access-level">
            <input type="checkbox" value="${c.id}" ${selected.has(c.id) ? 'checked' : ''}>
            <span>${String(c.code).replace(/^KZ-/, '')}</span>
          </label>
        `).join('')}
      </div>
    </section>
