...
// ===== Course access visual grouping: English / Қазақ тілі =====
function lpGroupAdminCourses(root=document){
  const boxes=[...root.querySelectorAll('input[type="checkbox"]')].filter(x=>{
    const v=(x.value||'').toUpperCase();
    const n=(x.name||'').toLowerCase();
    return /^(A1|A2|B1|B2|C1|KZ-A1|KZ-A2|KZ-B1|KZ-B2|KZ-C1)$/.test(v) ||
           n.includes('course');
  });
  if(boxes.length<2) return;

  const fieldset=boxes[0].closest('fieldset') || boxes[0].parentElement?.parentElement;
  if(!fieldset || fieldset.querySelector('.course-access-groups')) return;

  const groups=document.createElement('div');
  groups.className='course-access-groups';

  const makeGroup=(title,flag,items)=>{
    const g=document.createElement('div'); g.className='course-access-group';
    g.innerHTML=`<div class="course-access-group-title"><span class="flag">${flag}</span><span>${title}</span></div><div class="course-access-levels"></div>`;
    const levels=g.querySelector('.course-access-levels');
    items.forEach(input=>{
      let label=input.closest('label');
      if(!label){
        label=document.createElement('label');
        input.parentNode.insertBefore(label,input); label.appendChild(input);
      }
      label.classList.add('course-access-level');
      let code=(input.value||'').toUpperCase();
      if(code.startsWith('KZ-')) code=code.slice(3);
      // Keep checkbox and replace loose text with clean level code.
      [...label.childNodes].filter(n=>n.nodeType===3).forEach(n=>n.remove());
      if(![...label.children].some(x=>x.classList?.contains('level-name'))){
        const s=document.createElement('span'); s.className='level-name'; s.textContent=code; label.appendChild(s);
      }
  ...
  const en=boxes.filter(x=>!/^(KZ-)/i.test(x.value||''));
  const kz=boxes.filter(x=>/^(KZ-)/i.test(x.value||''));
  if(en.length) groups.appendChild(makeGroup('Английский язык','🇬🇧',en));
  if(kz.length) groups.appendChild(makeGroup('Қазақ тілі','🇰🇿',kz));
  fieldset.appendChild(groups);
}
const lpCourseObserver=new MutationObserver(()=>lpGroupAdminCourses());
lpCourseObserver.observe(document.documentElement,{childList:true,subtree:true});
document.addEventListener('DOMContentLoaded',()=>lpGroupAdminCourses());
setTimeout(()=>lpGroupAdminCourses(),300);
